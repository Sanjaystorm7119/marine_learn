...
- **Icons**: Lucide React (most components); `Learningpage.jsx` and `courses.jsx` each define 23 custom inline SVG icon wrappers
...
│   ├── mycourse.jsx           # My Courses grid (all 43 courses, no backend)
│   ├── calendar.jsx           # Activity heatmap calendar + streak tracker
│   ├── help.jsx               # Help center with FAQ accordion + search
...
**PublicLayout** (shows nav/footer on most routes, hides on `/login`, `/signup`, `/dashboard`, `/mycourses`, `/calendar`, `/admin/*`):
...
| `/mycourses` | `mycourse.jsx` | No nav/footer; grid of all 43 enrolled courses |
| `/calendar` | `calendar.jsx` | No nav/footer; activity heatmap + streak counter |
| `/help` | `help.jsx` | Help center with FAQ accordion |
...
#### `mycourse.jsx`
- Flattens all 43 courses from all 6 departments into a single grid
- No API calls — reads from `courseData.js` directly
- Framer Motion staggered card animations
- Color-coded department tags; clicking a card navigates to `/learn/:dept/:id`
- Reuses `sidenav.jsx` sidebar (same pattern as dashboard)

#### `calendar.jsx`
- Fetches `GET /users/me` on mount (auth required); redirects to `/login` if token fails
- **Activity heatmap:** month grid where each day shows green/red dot for active/inactive
- `generateMockData()` creates realistic daily activity logs (65% chance of activity per day) — all mock, no backend
- Selecting a day shows stats: lessons watched, time spent, quiz score, topics covered, achievements
- Month summary: active days %, modules completed, current streak
- Streak counter counts consecutive active days backward from today
- Reuses `sidenav.jsx` sidebar

#### `help.jsx`
- No API calls — entirely static content
- 4 help categories: Getting Started, Certificates, Account & Security, Courses & Content
- 8 FAQs with accordion expand/collapse; live search filters by question or answer text
- Contact support button has no backend integration
...
#### `sidenav.jsx`
- Reusable sidebar used across `dashboard.jsx`, `mycourse.jsx`, `calendar.jsx`, and others
- Reads `full_name` and `role` from `localStorage` for user display
- Nav items: Dashboard, My Courses, Calendar, Certificates, Reports, Resources, Settings, Help
- Reports and Resources navigation items exist in sidebar but link back to `/dashboard` (not implemented)
...
| `calendar.jsx` | GET | `http://localhost:8000/users/me` | Bearer |
...
> `mycourse.jsx`, `help.jsx`, `courses.jsx`, `Learningpage.jsx`, `Coursecatalog.jsx`, `certificate.jsx`, and `settings.jsx` make **no API calls**.
...
- python-jose 3.5, email-validator 2.3, cryptography 46
...
- Calendar activity data is mock (`generateMockData()`) — no backend integration
- AdminLayout sidebar shows "Course Management" and "System Settings" nav items that have no implemented routes
- `sidenav.jsx` sidebar shows "Reports" and "Resources" items that navigate to `/dashboard` (not implemented)
...
- `role` field in User model is a plain string with no enum validation — backend accepts any string value
