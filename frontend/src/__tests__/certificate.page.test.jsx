...
describe("Certificates page — API integration", () => {
  ...
  it("shows an error or empty state when the API returns a non-ok response", async () => {
    mockFetch({ detail: "Unauthorized" }, false);
    render(<Certificates />);

    // The component should not crash; it should show either loading→empty or an error
    await waitFor(() => {
      // Either shows empty state or doesn't crash; the certificate cards must not appear
      expect(screen.queryByText("ECDIS Fundamentals")).not.toBeInTheDocument();
    });
  });

  it("handles a network error (fetch rejection) without crashing", async () => {
    global.fetch = vi.fn().mockRejectedValue(new Error("Network Error"));
    render(<Certificates />);

    // Component must not throw; loading state disappears
    await waitFor(() => {
      expect(screen.queryByText(/ECDIS Fundamentals/)).not.toBeInTheDocument();
    });
  });

  it("shows correct user_full_name on expanded certificate card", async () => {
    mockFetch(MOCK_CERTS);
    render(<Certificates />);
    ...
    await waitFor(() => {
      // "Test Sailor" appears in the expanded card detail
      const matches = screen.getAllByText(/Test Sailor/i);
