...
import {
  ...
  ChevronDown, ChevronUp, Eye, Send, Award, TrendingUp, Filter,
  ChevronLeft, ChevronRight, BarChart3, Percent, Target, Flame, Plus, X
} from "lucide-react";
...
const AdminUserCourse = () => {
 ...
 const fetchData = async () => {
    setLoading(true);
    try {
      // Get token from local storage (checks both common names)
      const token = localStorage.getItem("access_token") || localStorage.getItem("token"); 
      
      const headers = {
        "Content-Type": "application/json",
        "Authorization": `Bearer ${token}` // Fixes the 401 Unauthorized error
      };

      // 1. Fetch Users
      const userRes = await fetch("http://127.0.0.1:8000/admin/user-courses", { headers });
      if (userRes.ok) {
      ...
      // 2. Fetch Courses
      const courseRes = await fetch("http://127.0.0.1:8000/study/modules", { headers });
      if (courseRes.ok) {
        const courseData = await courseRes.json();
        setCoursePool(courseData);
      } else {
        console.error("Course fetch failed:", await courseRes.text());
        setCoursePool([]); // <-- THIS PREVENTS THE BLANK PAGE CRASH!
      }
...
const handleAssignCourses = async () => {
    if (selectedUsersForAssign.length === 0 || selectedCoursesForAssign.length === 0) return;
    
    try {
      const token = localStorage.getItem("access_token") || localStorage.getItem("token");
      
      const response = await fetch("http://127.0.0.1:8000/admin/assign-courses", {
        method: "POST",
        headers: { 
          "Content-Type": "application/json",
        ...
        body: JSON.stringify({
          ...
          module_ids: selectedCoursesForAssign 
        })
      });

      if (response.ok) {
        _toastCallback && _toastCallback({
          title: "Courses Assigned",
          description: `Successfully assigned courses to selected users.`,
        });
  ...
  const availableCoursesForAssign = useMemo(() => {
    ...
    return coursePool.filter(course =>
      selectedUsersForAssign.some(uid => {
        ...
        return !user?.courses.some(uc => uc.courseId === course.id);
      })
    );
  ...
  const filteredAssignUsers = useMemo(() => {
    let list = users;
    if (assignTarget === "officer") list = list.filter(u => u.role === "officer");
    else if (assignTarget === "crew") list = list.filter(u => u.role === "crew");
    if (assignUserSearch) {
  ...
  const filtered = useMemo(() => {
    return users
      ...
      .filter((u) => {
        ...
        if (statusFilter === "in-progress") return u.overallProgress > 0 && u.overallProgress < 100;
        if (statusFilter === "not-started") return u.overallProgress === 0;
        if (statusFilter === "low-progress") return u.overallProgress > 0 && u.overallProgress < 30;
        return true;
      });
  ...
  const stats = useMemo(() => {
    ...
    const avgProgress = total
      ...
      : 0;
    const pending = roleUsers.filter((u) => u.overallProgress > 0 && u.overallProgress < 100).length;
  ...
  const quizBadge = (score) => {
    ...
    return (
      <span className={`quiz-score ${score >= 70 ? "quiz-pass" : "quiz-fail"}`}>{score}%</span>
    );
  };
  ...
  const statCards = [
    { label: "Total", value: stats.total, Icon: Users, colorClass: "stat-cyan" },
    { label: "Completed All", value: stats.completed, Icon: CheckCircle2, colorClass: "stat-emerald" },
    { label: "In Progress", value: stats.pending, Icon: Clock, colorClass: "stat-amber" },
    { label: "Avg Progress", value: `${stats.avgProgress}%`, Icon: TrendingUp, colorClass: "stat-purple" },
    { label: "Avg Quiz Score", value: `${stats.avgQuiz}%`, Icon: Target, colorClass: "stat-pink" },
  ];
  ...
  return (
    <div className="ucm-root">
      ...
      >
        ...
        <div className="ucm-header">
          <div>
           <h1 className="ucm-title">User Course Management</h1>
            <p className="ucm-subtitle">Track learner progress, assign courses, quiz scores &amp; send reminders</p>
          </div>
          <div className="header-actions">
            <button className="btn-assign-course" onClick={() => { setSelectedUsersForAssign([]); setSelectedCoursesForAssign([]); setAssignUserSearch(""); setAssignTarget("all"); setAssignDialogOpen(true); }}>
        ...
        <div className="tab-list">
          <button
            ...
            onClick={() => { setActiveTab("officer"); setPage(1); }}
          >
          ...
          <button
            ...
            onClick={() => { setActiveTab("crew"); setPage(1); }}
          >
        ...
        <div className="filter-row">
          ...
          <div className="custom-select-wrap">
            ...
            >
              ...
              <ChevronDown className={`icon-xs select-chevron ${selectOpen ? "chevron-open" : ""}`} />
            </button>
            {selectOpen && (
              <div className="custom-select-dropdown">
                {statusFilterOptions.map((opt) => (
                  <button
                    ...
                    onClick={() => { setStatusFilter(opt.value); setPage(1); setSelectOpen(false); }}
                  >
                    {opt.label}
                  </button>
                ))}
     ...
     {/* ── Assign Course Dialog ── */}
        {assignDialogOpen && (
          <div className="dialog-overlay" onClick={() => setAssignDialogOpen(false)}>
            <div className="dialog-box" onClick={(e) => e.stopPropagation()}>
              <div className="dialog-header">
                <div>
                  <h2 className="dialog-title">Assign Courses to Users</h2>
                  <p className="dialog-desc">Select users and courses to assign. Already-assigned courses are skipped.</p>
                </div>
                <button className="dialog-close" onClick={() => setAssignDialogOpen(false)}>
                  <X className="icon-sm" />
                </button>
              </div>
              ...
              <div className="dialog-body">
                ...
                <div className="dialog-section">
                  <div className="dialog-section-header">
                    ...
                    <div className="custom-select-wrap">
                      <button className="custom-select-trigger dialog-select-sm" onClick={() => setAssignTargetSelectOpen(v => !v)}>
                        <span>{{ all: "All Users", officer: "Officers Only", crew: "Crew Only" }[assignTarget]}</span>
                        <ChevronDown className={`icon-xs select-chevron ${assignTargetSelectOpen ? "chevron-open" : ""}`} />
                      </button>
                      {assignTargetSelectOpen && (
                        <div className="custom-select-dropdown">
                          {[{ value: "all", label: "All Users" }, { value: "officer", label: "Officers Only" }, { value: "crew", label: "Crew Only" }].map(opt => (
                            <button key={opt.value} className={`custom-select-item ${assignTarget === opt.value ? "select-item-active" : ""}`}
                              onClick={() => { setAssignTarget(opt.value); setSelectedUsersForAssign([]); setAssignTargetSelectOpen(false); }}>
                              {opt.label}
                            </button>
                          ))}
                        </div>
                  ...
                  <div className="search-wrap dialog-search">
                    <Search className="search-icon" />
                    <input className="search-input" placeholder="Search users..." value={assignUserSearch}
                  ...
                  <div className="assign-list">
                    {filteredAssignUsers.map(user => (
                      <label key={user.id} className="assign-list-item">
                        <input type="checkbox" className="assign-checkbox"
                        ...
                        <span className="assign-item-count">{user.courses.length} courses</span>
                      </label>
                    ))}
                  </div>
                  {selectedUsersForAssign.length > 0 && (
                    <p className="assign-selected-info assign-selected-cyan">{selectedUsersForAssign.length} user(s) selected</p>
                  )}
                </div>

                {/* Step 2 — Courses */}
                <div className="dialog-section">
                  <p className="dialog-section-label">2. Select Courses to Assign</p>
                 <div className="assign-list assign-list-courses">
                    {availableCoursesForAssign.map(course => (
                      <label key={course.id} className="assign-list-item">
                        <input type="checkbox" className="assign-checkbox assign-checkbox-emerald"
                          checked={selectedCoursesForAssign.includes(course.id)}
                          onChange={(e) => {
                            setSelectedCoursesForAssign(prev =>
                              e.target.checked ? [...prev, course.id] : prev.filter(c => c !== course.id)
                            );
                          }} />
                  ...
                  {selectedCoursesForAssign.length > 0 && (
                    <p className="assign-selected-info assign-selected-emerald">{selectedCoursesForAssign.length} course(s) selected</p>
                  )}
                </div>
              </div>

              <div className="dialog-footer">
                <button className="btn-dialog-cancel" onClick={() => setAssignDialogOpen(false)}>Cancel</button>
                <button className="btn-dialog-assign"
                  disabled={selectedUsersForAssign.length === 0 || selectedCoursesForAssign.length === 0}
                  onClick={handleAssignCourses}>
                  <Plus className="icon-sm" />
                  Assign {selectedCoursesForAssign.length > 0 ? `${selectedCoursesForAssign.length} Course(s)` : "Courses"}
                </button>
              </div>
            </div>
          </div>
...
const UserTable = ({ users, expandedUser, setExpandedUser, statusBadge, quizBadge, onNotify, onAssign }) => (
      ...
      <table className="ucm-table">
        ...
        <tbody>
          {users.map((user) => {
            ...
            const completedCount = user.courses.filter((c) => c.status === "completed").length;
                ...
                >
                  ...
                  <td className="td">
                    <div className="courses-cell">
                      ...
                      <span className="courses-total">{user.courses.length}</span>
                      {pendingCount > 0 && (
                        <span className="courses-pending">({pendingCount} pending)</span>
                      )}
                    </div>
                  </td>
                  <td className="td">
                    <div className="progress-cell">
                      ...
                      <span className="progress-label">{user.overallProgress}%</span>
                    </div>
                  </td>
                  <td className="td">
                  ...
                  <td className="td td-right">
                    <div className="actions-cell">
                      <button
                        ...
                        title="Assign course"
                        onClick={(e) => { e.stopPropagation(); onAssign(user.id); }}
                      >
                        <Plus className="icon-sm" />
                      </button>
                      <button
                      ...
                      {pendingCount > 0 && (
                        <button
                          ...
                          title="Push reminder"
                          onClick={(e) => { e.stopPropagation(); onNotify(user); }}
                        >
                                ...
                                <div key={course.courseId} className="course-item">
                                  <div className="course-info">
                                    ...
                                    <p className="course-meta">
                                      {course.completedModules}/{course.totalModules} modules &bull;{" "}
                                      {course.completedLessons}/{course.totalLessons} lessons &bull; Last:{" "}
                                      {course.lastAccessed}
                                    </p>
                                  </div>
                                  <div className="course-stats">
                                    ...
                                    <div className="course-quiz">{quizBadge(course.quizScore)}</div>
                                  </div>
                                </div>
                              ))}
                            </div>
          ...
          {users.length === 0 && (
            <tr>
              <td colSpan={7} className="empty-row">No users found matching your filters.</td>
            </tr>
          )}
        </tbody>
      </table>
